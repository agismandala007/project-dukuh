...
<head>
    ...
    <title>Tambah Anggota Padukuhan</title>
...
<body>
    ...
    <div class="flex flex-col gap-8 p-4 mx-auto mt-14">
        <div class="block w-full lg:w-3/6 mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow">
            <form class="mx-auto" action="{{ route('admin.anggota.store') }}" method="post"
                enctype="multipart/form-data">
                @csrf
                <div class="mb-5">
                    <label for="nama" class="block mb-2 text-sm font-medium text-gray-900">Nama</label>
                    <input type="text" id="nama" name="nama"
                        class="form-input  @error('nama') border-red-500 @enderror" value="{{ old('nama') }}" />
                    @error('nama')
                ...
                <div class="mb-5">
                    ...
                    <input type="number" id="nip" name="nip"
                        class="form-input  @error('nip') border-red-500 @enderror" value="{{ old('nip') }}" />
                    @error('nip')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="jabatan" name="jabatan"
                        class="form-input  @error('jabatan') border-red-500 @enderror" value="{{ old('jabatan') }}" />
                    @error('jabatan')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
